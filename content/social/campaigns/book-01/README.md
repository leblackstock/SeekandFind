...
- `npm run social:youtube-publish` runs the publish-enabled YouTube lane with compact stdout plus local full/compact evidence JSON and session logging. Use only when Publish is intentionally allowed and an existing YouTube Studio tab is already open; the helper must stop instead of navigating, opening a new tab, or closing the attached browser session.
- YouTube uploads must use the current Studio flow: open or reuse the Studio tab, click `Create`, click `Upload videos`, handle the `Select files` file chooser, fill Details, select `Yes, it's made for kids`, advance through Video elements and Checks, select `Public`, then publish or stop before Publish for dry runs. Do not rely on searching for a raw `input[type=file]` before clicking `Select files`; current YouTube Studio exposes the file picker through that button.
- If a YouTube upload tab becomes unresponsive to CDP Runtime or screenshots while an upload draft is open, stop automated posting and recover the visible draft manually or with a purpose-built no-navigation recovery helper. Do not reload, navigate away, or close the upload tab as a first response.
